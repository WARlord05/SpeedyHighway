...
class EnemyCar:
    ...
    def _default_choice(self, choices):
        """Default random choice (uses first item)."""
        import random
    ...
    def draw(self, surface):
        """Draw the enemy car on the given surface."""
        surface.blit(self.image, (self.x, self.y))
    
    def is_off_screen(self, screen_height):
        """Check if enemy car has moved off the bottom of screen."""
        return self.y > screen_height
    
    def reset(self, random_func=None):
        """Reset enemy car to top of screen in new lane."""
        if random_func:
    ...
    def get_rect(self):
        """Get enemy car bounding rectangle."""
    ...
    def check_collision(self, player_x, player_y, player_width):
        ...
        if (self.y + self.height > player_y and 
            self.y < player_y + 100):  # Approximate player height
    ...
    def check_near_miss(self, player_x, player_y, threshold_x=50, threshold_y=100):
        """Check if enemy car is in near-miss range with player."""
